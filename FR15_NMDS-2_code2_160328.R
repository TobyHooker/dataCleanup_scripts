...
plot(veg.stress)
veg1.plot <- plot(veg.nmds1) # recall the veg1.plot is now an ordiplot-object. additional work on the ordination should be based on the ordiplot-object (veg1.plot) and not the ordination-object (veg.nmds1)
...
row.names(veg2.dat) <- veg2.dat$key.dist
...
#############################
veg2.nmds2 <- metaMDS(veg2.dat3, distance="bray", k=2)   ## This is the Ordination ###
veg2.nmds2.3 <- metaMDS(veg2.dat3, distance="bray", k=3)
##### NMDS Calculations #####
## plot Shepard Plot ######
jpeg(filename="Veg2.nmds.Stress_1.jpeg", width=7.5, height=6, units="in", res=500)
    veg2.stress <- stressplot(veg2.nmds2)
    veg2.stress <- stressplot(veg2.nmds2.3)  ## new - added 160331
dev.off();
#####  NMDS Ordination Plot #####
veg2.plot1 <- plot(veg2.nmds2)  # can work from ordiplot <object> or nmds <object>
veg2.3.plot1 <- plot(veg2.nmds2.3)
veg2.3.plot1.13 <- plot(veg2.nmds2.3, choices=c(1,3))
veg2.3.plot1.23 <- plot(veg2.nmds2.3, choices=c(2,3))
...
nmds.scree <- function(x){
    plot(rep(1,10), replicate(10, metaMDS(x, k=1)$stress),
         ...
         main="NMDS Stress plot: Vegetation")
    for (i in 1:6) {
        points(rep(i+1, 10), replicate(10, metaMDS(x, k=i+1)$stress))   }   };

#pdf("NMDS_veg_SCree.pdf")
jpeg("NMDS_veg_Scree.jpeg", width=5, height=5, units="in", quality=80, res=500)
