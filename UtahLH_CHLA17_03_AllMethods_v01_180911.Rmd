---
title: "Utah Lake CHL-Method Comparison (2017) 3.0"
author: "T.Hooker"
...
+ 180911 :: _Add UPHL-HPLC method and run analyses_
...
_Note_ :: SK6 (SK1 & SampNum) generated [180912]

```{r data-combo-1, eval=FALSE}
uphl.vars <- c("MLID", "month", "Sample.Date", "SK1", "PARAM", "Result.Code", "CHL_result", "CHL_units", "CHL_Umdl", "CHL_Ulrl", "CHLA.vol", "SK6")
ctf.vars <- c("MLID", "month", "SAMPDATE", "SK1", "PARAM", "Result.Flag", "CHL_result", "UNITS", "DL", "RL", "CHLA.vol", "SK6")
#
# new SK6
uphl_CHL.dat1$SK6 <- paste(uphl_CHL.dat1$SK1, uphl_CHL.dat1$Sample.Number, sep=".")
uphl_1 <- uphl_CHL.dat1[,uphl.vars]
uphl_1$Lab <- "UPHL"
uphl_1 <- dplyr::arrange(uphl_1, SK6, PARAM)
## transform names to CTF version
names(uphl_1) <- c("MLID", "month", "SDate","SK1", "PARAM", "Flag", "CHL_result", "Units", "MDL", "MRL", "FVol","SK6","Lab")
## add REPs to results
for (k in uphl_1$SK1) {
...
####
# new SK6
ctf_CHL.dat1$SK6 <- paste(ctf_CHL.dat1$SK1, ctf_CHL.dat1$LABSAMPID, sep=".")
ctf_1 <- ctf_CHL.dat1[,ctf.vars]
ctf_1$Lab <- "CTF"
ctf_1 <- dplyr::arrange(ctf_1, SK6, PARAM)
#
names(ctf_1) <- c("MLID", "month", "SDate","SK1", "PARAM", "Flag", "CHL_result", "Units", "MDL", "MRL", "FVol","SK6","Lab")
## add REPs to results
for (k in ctf_1$SK1) {
...
### 4.0 Add 3rd CHL Method 

#### 4.1 Import Data

+ Data for HPLC analysis from UPHL was initially reviewed  as part of the 'New CHL Method Compilation'  [_180911_]

```{r import_hplc, eval=F}
# file_hplc <- choose.files(caption="Select DATA file [*.xlsx]", multi = FALSE)
hplc_01 <- openxlsx::read.xlsx(file_hplc, sheet=1, startRow = 1, colNames=T, rowNames=FALSE);  
```

+ File name :: `r basename(file_hplc)`
+ File path :: `r dirname(file_hplc)`

**Note that this datafile includes _all_ results available for CHL from UPHL and is not HPLC only**

#### 4.2 Filter to UTAH LAKE

**By MLID**

```{r mlid_recs}
utlk_mlid <- unique(CHLA.dat1$MLID)
nrow(hplc_01[hplc_01$MLID_x %in% utlk_mlid,])
#
hplc_02 <- hplc_01[hplc_01$MLID_x %in% utlk_mlid,]
rownames(hplc_02) <- NULL
```

+ `r nrow(hplc_02)` records w/ MLIDs for Utah Lake-CHL project
    + Date range :: from `r min(hplc_02$SDate_x)` to `r max(hplc_02$SDate_x)`
    + Some results outside of range of interest...

+ Utah Lake CHL data-range ::  from `r min(CHLA.dat1$SDate)` to `r max(CHLA.dat1$SDate)`

+ Utah Lake-CHL Sample Dates ::
    + `r paste(sort(unique(CHLA.dat1$SDate)), collapse = " / ")`

**Just build SK1 back into `hplc_02` and pull from there...**

```{r hplc_SK1}
if(!"tidyr" %in% (.packages())) {invisible(lapply(tidy_package, library, character.only=T, quietly = T, warn.conflicts = F))}
#
for (i in 1:nrow(hplc_02)) {
    hplc_02$SKey1[i] <- paste(stringi::stri_split_regex(hplc_02$SKey6[i],
                                            "[.]", n=3, simplify = TRUE)[,1:2],
                              collapse=".")   }
hplc_02 <- select(hplc_02, Lab.filename:PROJECT, SKey1, everything())
#
utlk_SK1 <- unique(CHLA.dat1$SK1)
hplc_03 <- hplc_02[hplc_02$SKey1 %in% utlk_SK1,]
#
# (unique(hplc_03$SKey1)) %in% (unique(CHLA.dat1$SK1))
```


+ Working datafile :: `hplc_03`
...
CHLA.dat2 <- CHLA.dat1
CHLA.dat2$Set <- "one"
var1 <- names(CHLA.dat2)
#
if(!"tidyr" %in% (.packages())) {invisible(lapply(tidy_package, library, character.only=T, quietly = T, warn.conflicts = F))}
#
# select vars for new data
hplc_03set <- select(hplc_03, MLID_x, SDate_x, SKey1, PARAM, Result.Code, RV2,
                     Units2, Method.Detect.Limit, Lower.Report.Limit,SKey6, Method_x)
#
# add remaining cols for matchup
hplc_03set$month <- lubridate::month(as.Date(hplc_03set$SDate_x))


## add REPs to results
## block below takes too long to process (inefficient code)
# for (h in hplc_03set$Method_x) {
#     for (k in hplc_03set$SKey1) {
#         for (m in hplc_03set$PARAM) {
#             hplc_03set$REP[hplc_03set$Method_x == h & hplc_03set$SKey1 == k & hplc_03set$PARAM == m] <-
#                 1:length(hplc_03set$SKey1[hplc_03set$Method_x == h & 
#                                               hplc_03set$SKey1 == k & hplc_03set$PARAM == m])
#             }   }   }
hplc_03set <- dplyr::arrange(hplc_03set, SKey6, PARAM, Method_x)
hplc_03set$Set <- "two" 
#
CHLA.dat2$SDate <- as.character(CHLA.dat2$SDate)
# fix varnames
names(hplc_03set) <- c("MLID", "SDate", "SK1", "PARAM", "Flag", "CHL_result",
                       "Units", "MDL", "MRL", "SK6", "Lab", "month", "Set")
#
CHL_data <- bind_rows(CHLA.dat2, hplc_03set)

```

**Working dataframe `CHL_data`**

#### 4.4 Examine Combined datasets

```{r adat.01}
if(!"tidyr" %in% (.packages())) {invisible(lapply(tidy_package, library, character.only=T, quietly = T, warn.conflicts = F))}
#
CHL_data <- select(CHL_data, MLID:SK1, SK6, everything())
CHL_data <- arrange(CHL_data, SK1, PARAM, SK6, Lab, Set)
# align PARAM names
addmargins(table(CHL_data$PARAM, CHL_data$Lab, useNA = "ifany"))
#
CHL_data$PARAM <- plyr::revalue(CHL_data$PARAM,
                                c("CHL.tot" = "CHL_tot"))
CHL_data$PARAM <- ifelse(CHL_data$PARAM == "CHL_tot" & CHL_data$Units == "ug/L",
                         "CHLtot.c", CHL_data$PARAM)
# PARAM x Data.Src X Result.Unit
tab1 <- as.data.frame.matrix(addmargins(
    table(CHL_data$PARAM, 
        CHL_data$Lab, 
        useNA = "ifany"),1))
#
tab1x <- as.data.frame.matrix(addmargins(
    table(paste(CHL_data$PARAM, " [ ", CHL_data$Units, " ]"), 
        CHL_data$Lab, 
        useNA = "ifany"),1))
#
tab1$Units <- NA
tab1$PARAM <- rownames(tab1); rownames(tab1) <- NULL; tab1x$PARAM <- rownames(tab1x); rownames(tab1x) <- NULL
tab1 %<>% select(PARAM, everything()); tab1x %<>% select(PARAM, everything())
#
for (k in tab1$PARAM[1:(nrow(tab1)-1)]) {
    tab1$Units[tab1$PARAM == k] <- 
        paste(unique(CHL_data$Units[CHL_data$PARAM == k]), collapse = " & ")   }
```

```{r adat01.tab}
# Table : PARAM x Lab w/ Units
tab1$Comment <- NA
tab1$Comment[tab1$PARAM == "CHLA"] <- "Mixed data here"
tab1$Comment[tab1$PARAM == "CHLtot.c"] <- "Same Units"
tab1$Comment[tab1$PARAM == "PHEOA"] <- "Mixed data here"
# styling for missing / zero's (0)
tabdat <- tab1
css.cell <- matrix("", nrow=nrow(tabdat)+1, ncol=ncol(tabdat))
cellbold <- matrix("", nrow=nrow(tabdat)+1, ncol=ncol(tabdat)-2)
css.x <- matrix("", nrow=nrow(tabdat)+1, ncol=ncol(tabdat))
for (i in 2:(nrow(cellbold)-1)) {
    for (j in 2:ncol(cellbold)) {
        cellbold[i,j] <- ifelse(is.na(tabdat[i-1,j]), "font-weight: 700;background: gray90; border: 1px solid;",
            ifelse(tabdat[i-1,j] != 0, "font-weight: 700;background: #DCDCDC; border: 1px solid;", ""))     }   }
css.cell = rbind(rep("padding-left: 0.7em; padding-right: 0.9em;font-size: 0.8em;", times=ncol(tabdat)), matrix("padding-left: 0.8em; padding-right: 0.8em; font-size: 0.8em;",ncol=ncol(tabdat), nrow=nrow(tabdat)))
for (i in 1:nrow(cellbold)) {
    for (j in 1:ncol(cellbold)) {
        css.cell[i,j] <- paste(css.cell[i,j], cellbold[i,j])  }   }
#
htmlTable::htmlTable(tabdat, css.cell = css.cell, caption = "Parameters by data-source w/ Units in combined dataset", rnames=FALSE, total = TRUE, align = c("l|cccc|ll"))
```

```{r adat01.tab2}
# Table : [PARAM x Units] x Lab  #####   ######
# styling for missing / zero's (0)
tabdat <- tab1x
css.cell <- matrix("", nrow=nrow(tabdat)+1, ncol=ncol(tabdat))
cellbold <- matrix("", nrow=nrow(tabdat)+1, ncol=ncol(tabdat))
...
for (i in 2:(nrow(cellbold)-1)) {
    for (j in 2:ncol(cellbold)) {
        cellbold[i,j] <- ifelse(is.na(tabdat[i-1,j]), "font-weight: 700;background: gray90; border: 1px solid;",
            ifelse(tabdat[i-1,j] != 0, "font-weight: 700;border: 0.5px solid;", ""))     }   }
# background: #DCDCDC; 
css.cell = rbind(rep("padding-left: 0.7em; padding-right: 0.9em;font-size: 0.8em;", times=ncol(tabdat)), matrix("padding-left: 0.8em; padding-right: 0.8em; font-size: 0.8em;",ncol=ncol(tabdat), nrow=nrow(tabdat)))
for (i in 1:nrow(cellbold)) {
    for (j in 1:ncol(cellbold)) {
        css.cell[i,j] <- paste(css.cell[i,j], cellbold[i,j])  }   }
#
htmlTable::htmlTable(tabdat, css.cell = css.cell, caption = "PARAM x Unit vs. data-source in combined dataset", rnames=FALSE, total = F, align = c("l|cccc"),
    n.tspanner=c(1,1,2,1,2,1,2,1,1,1), tspanner=c("", "", "", "", "", "", "", "", "",""),
    css.rgroup = "font-weight: 700; text-align: center; font-style: italic;",
    css.tspanner.sep = "border-top: 1px solid;")
```

+ Three parameters have mixed Units
    + CHLA includes both weight (ug) and concentration (ug/L)
    + CHLtot.c is _only_ concentration (but units style differs)
    + PHEOA includes both weight (ug) and concentration (ug/L)

**Key PARAMs**

1) CHLA -- concentation
2) CHLtot.c -- concentration
3) PHEOA -- less important, as concentration

**Comparable PARAM x Lab(method) combinations**

+ CHLA (concentration, as ug/L)
    + `CHLA` & Lab = ["CTF", "UPHL"]
    + `CHLA.c` & Lab = ["10200H", "HPLC"]

+ CHL [total] (concentation, as ug/L)
    + `CHLtot.c` & Lab = ["10200H", "HPLC"] ( _where units = "UG-L"_ )
    + `CHLtot.c` & Lab = ["UPHL"] ( _where units = "ug/L"_ )
    + **CHL-Total concentration for CTF needs manual calculation**

  
+ Working datafile :: `CHL_data`

***

#### 4.5 Fix filtration vols

```{r adat.02}
CHL_data_copy <- CHL_data

for (j in unique(CHL_data$SK1)) {
    dat_j <- CHL_data[CHL_data$SK1 == j,]
    CHL_data$FVol[CHL_data$SK1 == j] <- 
        ifelse(length(unique(dat_j$FVol[dat_j$SK1 == j & !is.na(dat_j$FVol)])) == 1,
               unique(dat_j$FVol[dat_j$SK1 == j & !is.na(dat_j$FVol)]),
               ifelse(length(unique(dat_j$FVol[dat_j$SK1 == j & !is.na(dat_j$FVol)])) == 0,
                      NA,
                      NA))  }
#
CHL_data$FVol[CHL_data$SK1 == "4930009.43089"] <- CHL_data$CHL_result[CHL_data$PARAM == "filt.VOL" & CHL_data$SK1 == "4930009.43089"]
#
CHL_data %>% group_by(SK1) %>%
    summarize(., meanVol = mean(FVol, na.rm=T),
              sdVol = sd(FVol, na.rm=T),
              nObs = n())
```

+ [180913] Vols looks good

#### 4.6 Trim Dataset

+ Date Range :: From `r min(CHL_data$SDate, na.rm=T)` to `r max(CHL_data$SDate, na.rm=T)`
...
+ Dates look okay atm (180913)

***

### 5.0 Method Comparisons

#### 5.1 CHL-A concentrations
...
#
CHLA01 <- CHL_data %>%
    filter((PARAM == "CHLA" & (Lab == "CTF" | Lab == "UPHL")) |
               (PARAM == "CHLA.c" & (Lab == "10200H" | Lab == "HPLC")))
# clean up "REPs"
for (j in unique(CHLA01$SK6)) {
    CHLA01$REP[CHLA01$SK6 == j] <- CHLA01$REP[CHLA01$SK6 == j & !is.na(CHLA01$REP)]
}
# j = "4917310.42927.2070677"
```

```{r fig01_data}
# trim sites (not 4917320 & 4930009)
plot1_dat <- CHLA01[(CHLA01$MLID != 4917320 & CHLA01$MLID != 4930009),]
# cast the data by LAb
plot1_dat2 <- reshape2::dcast(plot1_dat, MLID + month + SK1 + REP ~ Lab, 
                             value.var = "CHL_result",
                             fun.aggregate=function(x) mean(x, na.rm=TRUE))
# rename 10200H
plot1_dat2 %<>% dplyr::rename("UVVIS" = "10200H")
```
...
z1
# UPHL (uvvis) vs 10200H (uvvis) - Data check
z2 <- z + geom_point(aes(x = UPHL, y = UVVIS), na.rm=TRUE) + 
    geom_abline(slope = 1, intercept = 0, lty=2, col = "black") + 
    theme_bw() + scale_x_continuous(limits= c(0,NA)) + 
    scale_y_continuous(limits=c(0,NA)) + 
    ggtitle("Scatterplot of CHLA conc. (ug/L) :: UPHL versus UPHL")
z2
# (plot1_dat2[(!is.na(plot1_dat2$UVVIS) & 
                # !is.na(plot1_dat2$UPHL) &
                # plot1_dat2$UVVIS != plot1_dat2$UPHL),])
# UPHL (uvvis) vs HPLC
z3 <- z + geom_point(aes(x = UPHL, y = HPLC), na.rm=TRUE) + 
    geom_abline(slope = 1, intercept = 0, lty=2, col = "black") + 
    theme_bw() + scale_x_continuous(limits= c(0,NA)) + 
    scale_y_continuous(limits=c(0,NA)) + 
    ggtitle("Scatterplot of CHLA conc. (ug/L) :: UPHL UVVIS vs HPLC")
z3
# CTF vs HPLC
z4 <- z + geom_point(aes(x = HPLC, y = CTF), na.rm=TRUE) + 
    geom_abline(slope = 1, intercept = 0, lty=2, col = "black") + 
    theme_bw() + scale_x_continuous(limits= c(0,NA)) + 
    scale_y_continuous(limits=c(0,NA)) + 
    ggtitle("Scatterplot of CHLA conc. (ug/L) :: CTF UVVIS vs HPLC")
z4

```


#### 5.3 Aggregate Replicates (UTLK only)

```{r agg_01}
CHLA01$Lab[CHLA01$Lab == "10200H"] <- "UVVIS"
CHLA_SUMM01 <- plyr::ddply(CHLA01[(CHLA01$MLID != 4917320 & CHLA01$MLID != 4930009),
                    c("SK1","MLID", "month","SDate", "CHL_result", "Flag", "Lab")],
                           .(SK1, MLID, month, SDate, Lab), 
                           dplyr::summarize,
                           mean = round(mean(CHL_result, na.rm=T), 2),
                           sd = round(sd(CHL_result, na.rm=T), 2),
                           CV = round(sd / mean *100, 2),
                           ND = sum(!is.na(Flag)))
#
```

```{r agg01_fig01, fig.width=7, fig.height=9}
if(!"ggplot2" %in% (.packages())) {invisible(lapply(graph_package, library, character.only=T, quietly = T, warn.conflicts = F))}
#
zz <- ggplot(data = CHLA_SUMM01[CHLA_SUMM01$Lab != "UVVIS",], 
            aes(x = ((month)), y = mean, fill=Lab),na.rm=TRUE) + theme_bw() + 
    scale_fill_manual(values=c("white", "gray65", "blue")) +
    facet_wrap(~MLID, ncol=3, scales="free_y")
zz1 <- zz + geom_col(na.rm = TRUE, position=position_dodge(),col="black", width = 0.9) +
    geom_errorbar(aes(ymin=mean-sd, ymax=mean+sd), width = 0.3, 
                          position = position_dodge(0.9), na.rm=TRUE) +
    xlab("Month of Sample Collection") + ylab("Mean CHLA conc. (ug/L) [3 replicates]") +
    ggtitle("Mean Chlorophyll-A concentrations in Utah Lake: \nComparison of laboratory methods") + 
    theme(plot.title = element_text(hjust = 0.5),
          legend.position=c(0.95,0.07), legend.justification = c(0.8,0.5),
          legend.direction = "vertical", legend.title=NULL) +
    scale_x_continuous(breaks = c(5,6,7,8,9,10,11), 
                       labels=c("M", "Jn", "Jl", "Au", "Sp", "Oc","No")) +
    labs(fill=NULL)
zz1
...
```{r agg01_fig02_data}
CHLA_SUMM01_melt <- reshape2::melt(CHLA_SUMM01,
                                   id.vars = c(1:5),
                                   measure.vars = c(6:9),
                                   variable.name = "VAR")
CHLA_SUMM01_melt$LabVar <- paste(CHLA_SUMM01_melt$Lab, CHLA_SUMM01_melt$VAR, sep=".")
# re-cast
CHLA_SUMM02 <- reshape2::dcast(CHLA_SUMM01_melt[, c(1:5, 7, 8)],
                               SK1 + MLID + month + SDate ~ LabVar,
                               value.var = "value")
```
...
if(!"ggplot2" %in% (.packages())) {invisible(lapply(graph_package, library, character.only=T, quietly = T, warn.conflicts = F))}
# UPHL x CTF
q <- ggplot(data = CHLA_SUMM02, aes(x = UPHL.mean, y = CTF.mean), na.rm=TRUE) + 
    theme_bw() + scale_colour_manual(values=rainbow(7))
q1 <- q + geom_errorbar(aes(ymin=CTF.mean-CTF.sd, ymax=CTF.mean+CTF.sd, 
    ...
    geom_text(aes(label=MLID), size=2, vjust=2, hjust=0.5, na.rm=T, check_overlap = TRUE,
              nudge_x=4) +
    ggtitle("Pairwise comparison of mean CHLA conc.: UPHL vs. CTF") + 
    theme(plot.title = element_text(hjust = 0.5)) +
    labs(fill = "Sample Month")
...
if(!"ggplot2" %in% (.packages())) {invisible(lapply(graph_package, library, character.only=T, quietly = T, warn.conflicts = F))}
# UPHL x UVVIS
q <- ggplot(data = CHLA_SUMM02, aes(x = UPHL.mean, y = UVVIS.mean), na.rm=TRUE) + 
    theme_bw() + scale_colour_manual(values=rainbow(7))
q2 <- q + geom_errorbar(aes(ymin=UVVIS.mean-UVVIS.sd, ymax=UVVIS.mean+UVVIS.sd, 
                            width=0.3), na.rm=TRUE) +
    geom_errorbarh(aes(xmin=UPHL.mean-UPHL.sd, 
                       xmax=UPHL.mean+UPHL.sd, height=0.3), na.rm=TRUE) + 
    geom_point(aes(fill=factor(month)), na.rm=T, shape=21, size=4) + 
    ...
    geom_text(aes(label=MLID), size=2, vjust=2, hjust=0.5, na.rm=T, check_overlap = TRUE,
              nudge_x=4) +
    ggtitle("Pairwise comparison of mean CHLA conc.: UPHL vs. UVVIS") + 
    theme(plot.title = element_text(hjust = 0.5)) +
    labs(fill = "Sample Month")
q2
...
if(!"ggplot2" %in% (.packages())) {invisible(lapply(graph_package, library, character.only=T, quietly = T, warn.conflicts = F))}
# HPLC x CTF
q <- ggplot(data = CHLA_SUMM02, aes(x = HPLC.mean, y = CTF.mean), na.rm=TRUE) + 
    theme_bw() + scale_colour_manual(values=rainbow(7))
q2 <- q + geom_errorbar(aes(ymin=CTF.mean-CTF.sd, ymax=CTF.mean+CTF.sd, 
                            width=0.3), na.rm=TRUE) +
    geom_errorbarh(aes(xmin=HPLC.mean-HPLC.sd, 
                       xmax=HPLC.mean+HPLC.sd, height=0.3), na.rm=TRUE) + 
    geom_point(aes(fill=factor(month)), na.rm=T, shape=21, size=4) + 
    geom_abline(slope=1, intercept=0, lty=2, col="black") + 
    scale_x_continuous(limits= c(NA,NA)) + 
    scale_y_continuous(limits=c(NA,NA)) +
    geom_text(aes(label=MLID), size=2, vjust=2, hjust=0.5, na.rm=T, check_overlap = TRUE,
              nudge_x=4) +
    ggtitle("Pairwise comparison of mean CHLA conc.: HPLC vs. CTF") + 
    theme(plot.title = element_text(hjust = 0.5)) +
    labs(fill = "Sample Month")
q2
...
```{r export_data, eval=F}
CHLmethods_dat <- list("Data_all" = CHL_data,
                       "Data_CHLA" = CHLA01,
                       "CHLA_summ01" = CHLA_SUMM01,
                       "CHLA_summ02" = CHLA_SUMM02)
openxlsx::write.xlsx(CHLmethods_dat, 
